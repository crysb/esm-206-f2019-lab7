...
**Statement of test outcome, using two-sample, one-sided paired t-test:** 

"Mean lead concentration (ppb) measured in water collected after 2 minutes of flushing (`r round(mean_flushed, 2)` $\pm$ `r round(sd_flushed, 2)`, n = `r n_flushed`) was significantly less than lead concentration in immediately collected water samples (`r round(mean_immediate, 2)` $\pm$ `r round(sd_immediate, 2)`, n = `r n_immediate`) by a paired two-sample, one-sided t-test (t(`r round(flushed_less_ttest$parameter, 2)`) = `r round(flushed_less_ttest$statistic, 2)`, *p* < 0.001)."
